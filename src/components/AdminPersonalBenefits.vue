...
<script>
  ...
  export default{
    ...
    created(){
      ...
      this.initData(userInfo.sm_pi_ProxyInfoID)
    },
    methods: {
      getSummaries(param) {
      ...
      initData(id,page){
        let options = {
          ...
          "operateUserName": "",
          "pcName": "",
          "proxyID": '18111729770',    //供应商商户号
          "agentType": "",     //查询代理商  指定供应商类型下的订单 (0旅行社订单1车票订单2门票订单,3美食订单 留空或不传为所有)
          "balanceState": "",//结算类型  不传或传空值查所有 0 查未结算 1已结算
          page:page?page:1,
          rows:5
        }
        this.isLoading = true;
